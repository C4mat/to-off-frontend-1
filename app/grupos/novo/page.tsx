...
export default function NovoGrupoPage() {
      ...
      try {
        ...
        if (response.data) {
          ...
          if (response.data.length === 1) {
            // Se só tem uma empresa, já seleciona ela
            setFormData(prev => ({
              ...prev,
              cnpj_empresa: response.data[0].cnpj.toString()
            }))
          }
        }
      } catch (error) {
  ...
  const handleEmpresaChange = (value: string) => {
    setFormData((prev) => ({ ...prev, cnpj_empresa: value }))
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true)
    try {
      const response = await apiClient.createGrupo({
        nome: formData.nome,
        descricao: formData.descricao,
        telefone: formData.telefone,
        cnpj_empresa: Number(formData.cnpj_empresa),
      })
      ...
      if (response.data) {
        ...
        toast({
          title: "Erro",
          description: response.error || "Não foi possível criar o grupo",
          variant: "destructive",
        })
      }
    } catch (error) {
  ...
  return (
    <RouteGuard requiredUserType="rh">
      <AppLayout title="Novo Grupo" subtitle="Adicionar um novo grupo ao sistema">
